...
def compute_station_metrics(
    ...
    Returns:
        Dict of parallel arrays: distanceKm, elevationDeg, lossDb,
        doppler, azimuthDeg, r0, fG, theta0, wind, aod, abs.
    """
    atmo = atmosphere or {}
    ...
    for pt in data_points:
        ...
        gl = geometric_loss(
            los["distanceKm"],
            optics.get("satAperture", 0.6),
            optics.get("groundAperture", 1.0),
            optics.get("wavelength", 810),
        )
        dop = doppler_factor(
            station, pt["r_ecef"], pt["v_ecef"],
            optics.get("wavelength", 810),
        )
        ...
        out["elevationDeg"].append(los["elevationDeg"])
        out["lossDb"].append(gl["lossDb"])
