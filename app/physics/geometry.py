...
from .sky_background import background_noise_from_sky
...
def geometric_loss(
    ...
    Two divergence conventions are supported:

    - ``model="airy"`` (default): full-angle divergence θ = 1.22 λ / D_t
      (Airy first-null), the historical SimulCTTC model.
    - ``model="gaussian"``: Gaussian-beam half-width divergence
      w₀ = 2 λ / (π D_t) — the convention of Ntanos et al. 2021
      (Photonics 8, 544), Eq. (3), (5), (6).  The transmit/receive gains and
      free-space loss G_t·G_r·L_fsl collapse to the geometric capture ratio
      ``coupling = (D_r / (2 d w₀))²`` (spot radius at range d is d·w₀).
    ...
    d_t = max(sat_aperture_m, 1e-3)
    if (model or "airy").strip().lower() == "gaussian":
        w0 = 2.0 * lam / (math.pi * d_t)           # half-width divergence (rad)
        spot_r = max(w0 * dist_m, 1e-6)            # 1/e beam radius at range d
    else:
        divergence = 1.22 * lam / d_t              # full-angle divergence (rad)
...
def compute_station_metrics(
    ...
    *,
    ...
    # Formula-variant selectors (Ntanos 2021 paper-mode vs. legacy defaults)
    #   geometric_model: "airy" (default) | "gaussian" (Ntanos Eq. 3/5/6)
    #   pointing_model:  "gaussian" (default QUARC) | "beta" (Ntanos Eq. 8-10)
    geo_model = str(lb.get("geometric_model", "airy")).strip().lower()
    pointing_model = str(lb.get("pointing_model", "gaussian")).strip().lower()
    scint_p0_pt = scint_p0  # reuse outage prob for the beta pointing model

    # New physics flags (backward-compatible: default off)
    pat_fading_on = lb.get("pat_fading_enabled", False)
    dynamic_bg_on = lb.get("dynamic_background_enabled", False)
    ...
    out: Dict[str, List[float]] = {
        ...
        # beam wander / PAT fading / dynamic sky
        "beamWanderUrad": [], "patFadingDb": [], "skyRadianceWm2srUm": [],
        # turbulence statistics behind scintLossDb, kept for the Monte Carlo
        # channel (divergence is constant here but stays per-sample so a future
        # elevation-dependent beam model needs no contract change).
        "sigmaR2": [], "apertureAvg": [], "divergenceRad": [],
        # Effective 1-sigma pointing jitter actually used for the fading
        # penalty: PAT alone on a downlink, PAT and beam wander in quadrature
        # on an uplink (see combined_pointing_fading_db).  Surfaced so the MC
        # sampler reproduces the deterministic mean exactly instead of
        # re-deciding how the two contributions combine.
        "pointingJitterUrad": [],
    }
    ...
    if epoch_iso:
        try:
            ...
            logger.warning("Failed to parse epoch for sun/eclipse calculation — skipping", exc_info=True)

    for pt in data_points:
        los = los_elevation(station, pt["r_ecef"])
        elev = los["elevationDeg"]
        
        # Minimum elevation threshold for link establishment (avoids asymptotic losses near horizon)
        min_elev = lb.get("min_elevation_deg", 0.0)
        has_los = elev > min_elev

        # Geometric loss uses tx/rx apertures based on link direction
        gl = geometric_loss(los["distanceKm"], tx_ap, rx_ap, wl_nm, model=geo_model)
        
        # Doppler and atmosphere are pure geometry/optics — always defined.
        # Visibility is expressed via has_los → linkEstablished, not by
        # nulling the physics arrays (preserves the numeric data contract).
        dop = doppler_factor(
            station, pt["r_ecef"], pt["v_ecef"], wl_nm,
        )
        atm = _scale_atmosphere(
            elev, r0_z, fG_z, th0_z, w_rms, aod_z, abs_z,
        )
        ...
        # Pointing / PAT fading
        # When pat_fading_on: use Rayleigh-averaged fading with beam wander
        # in quadrature (uplink only). Otherwise: deterministic pointing loss.
        # Source: Bourgoin et al., Opt. Express 21, 23 (2013);
        #         Andrews & Phillips (2005) Eq. 12.46.
        if pat_fading_on and pe_urad > 0:
            bw_rad2 = beam_wander_variance_rad2(
                elev, wl_nm, tx_ap, atm["r0"], sat_alt_m, link_direction,
            )
            bw_rms_urad = math.sqrt(max(bw_rad2, 0.0)) * 1e6  # rad -> urad
            p_db = combined_pointing_fading_db(
                pe_urad, bw_rms_urad, divergence_rad, link_direction,
            )
        elif pointing_model == "beta" and pe_urad > 0:
            # Ntanos 2021 Eq. 8-10: beta-distributed jitter, outage p0.
            # w0 = Gaussian half-width divergence = 2λ/(πD_tx).
            bw_rms_urad = 0.0
            w0_rad = 2.0 * lam_m / (math.pi * max(tx_ap, 1e-3))
            p_db = pointing_loss_beta_db(pe_urad, w0_rad, scint_p0_pt)
        else:
            bw_rms_urad = 0.0
            p_db = pointing_loss_db(pe_urad, divergence_rad)
        # Keep the full turbulence statistics, not just the fade margin: a
        # Monte Carlo channel must draw from the SAME irradiance distribution
        # this margin is the p0 quantile of, so sigma_R^2 and the aperture-
        # averaging factor have to survive to the solver instead of being
        # recomputed there from slightly different inputs.
        if scint_on:
            sc = scintillation_stats(
            ...
            sc = {"sigma_r2": 0.0, "aperture_avg": 1.0}
            s_db = 0.0
        # Fixed optical loss only applies on an established link.
        f_db = fixed_db if has_los else 0.0
        ...
        # ── Background noise: dynamic sky model or static H_rad ──
        # Source: Pirandola et al., Adv. Opt. Photon. 12, 1012 (2020);
        #         Krisciunas & Schaefer (1991) for lunar component.
        sky_hrad = bg_hrad  # default: static config value
        if dynamic_bg_on and epoch_dt is not None and has_los:
            # Solar zenith from sun ECI direction vs station zenith
            sta_mag = math.sqrt(sum(c * c for c in sta_eci))
            if sta_mag > 0:
                cos_sz = sum(s * st / sta_mag for s, st in zip(sun_dir, sta_eci))
                solar_z = math.degrees(math.acos(max(-1.0, min(1.0, cos_sz))))
            else:
                solar_z = 90.0
            from datetime import datetime as _dt, timezone as _tz
            t_s = pt.get("t", 0.0)
            pt_dt = _dt.fromtimestamp(epoch_dt.timestamp() + t_s, tz=_tz.utc)
            sky_hrad = background_noise_from_sky(
                solar_z, station["lat"], station["lon"],
                station_alt_m, pt_dt, wl_nm,
            )
        bg_cps = (
            background_noise_cps(sky_hrad, bg_fov, rx_ap, bg_dlam, wl_nm)
            if bg_on and has_los else 0.0
        )
        ...
        margin = None
        link_ok = has_los
        if has_los and tx_pwr_dbm is not None:
        ...
        out["beamWanderUrad"].append(bw_rms_urad)
        out["patFadingDb"].append(p_db)
        out["skyRadianceWm2srUm"].append(sky_hrad if dynamic_bg_on else bg_hrad)

        out["sigmaR2"].append(sc["sigma_r2"])
        out["apertureAvg"].append(sc["aperture_avg"])
        out["divergenceRad"].append(divergence_rad)
        out["pointingJitterUrad"].append(
            math.sqrt(pe_urad ** 2 + bw_rms_urad ** 2)
            if is_uplink else pe_urad
        )
