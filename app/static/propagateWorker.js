...
function secularRates(a, e, iRad, j3Enabled, j4Enabled) {
  // Enhanced J2 secular rate computation with proper handling
  // Uses semi-latus rectum for more accurate calculations.
  // Mirrors app/physics/propagation.py:compute_j2_secular_rates +
  // compute_enhanced_secular_rates (J4 = Kozai 1959; J3 = long-period approx).
  ...
  const factor = -1.5 * J2 * rp * rp * n;

  // RAAN precession rate (rad/s)
  let dotOmega = factor * cosI;

  // Argument of perigee rate (rad/s)
  let dotArgPerigee = factor * (2.5 * s2 - 2.0);
  ...
  if (j3Enabled && Math.abs(e) > 1e-6) {
    ...
    dotArgPerigee += f3 * sinI * (5 * cosI * cosI - 1) / e;
  }
  if (j4Enabled) {
    const e2 = e * e;
    const rp4 = Math.pow(rp, 4);
    dotOmega += (15 / 32) * n * J4 * rp4 * cosI * (8 + 12 * e2 - (14 + 21 * e2) * s2);
  ...
  const dotMeanAnomaly = factor * eta * (1.5 * s2 - 1.0);
...
onmessage = async function(ev) {
  ...
  if (msg.type === 'propagateBatch') {
    const { constellation, timeline, epoch, j2Enabled, j3Enabled, j4Enabled } = msg.payload || {};
    ...
    for (let s = 0; s < total; s += 1) {
      ...
      if (j2Enabled) {
        const rates = secularRates(a, e, i, j3Enabled, j4Enabled);
