...
// ── Paper reproduction (Ntanos et al. 2021) ──────────────────────────────
export async function paperPreset() { return _get('/api/paper/preset'); }
export async function paperLinkSweep(payload) { return _post('/api/paper/link-sweep', payload || {}); }
export async function paperRadianceSweep(payload) { return _post('/api/paper/radiance-sweep', payload || {}); }
export async function paperSinglePass(payload) { return _post('/api/paper/single-pass', payload || {}); }
export async function paperConstellation(payload) { return _post('/api/paper/constellation', payload || {}); }

// ── Constellation-over-network study (contention-aware) ─────────────────
// POST /api/study/constellation — M satellites x N stations through the same
// physics path /api/solve uses, then scored under each contention policy.
export async function studyConstellation(payload) {
  return _post('/api/study/constellation', payload);
}
...
export const api = {
  ...
  paperPreset, paperLinkSweep, paperRadianceSweep, paperSinglePass, paperConstellation,
  studyConstellation,
};
