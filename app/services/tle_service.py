...
class TleService:
    ...
    ENDPOINT = CELESTRAK_BASE_URL + "/NORAD/elements/gp.php"
    ...
    def get_group(self, group_id: str) -> Dict[str, Any]:
        ...
        logger.info("Fetching TLE group: %s", group_id)
        try:
            payload = self._download(norm)
        except TleServiceError as exc:
            logger.warning("TLE fetch failed for %s: %s", group_id, exc)
            raise
        exp = now + self._ttl
